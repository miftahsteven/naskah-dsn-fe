        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
             <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
                ...
                {doc.status === 'DRAFT' && (
                  <button className="shrink-0 flex items-center justify-center gap-2 px-6 py-4 sm:py-3 gradient-primary text-white font-bold rounded-2xl shadow-lg shadow-primary/20 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all w-full md:w-auto">
                    <Play size={20} />
                   ...
                   {doc.versions.map((v: any) => (
                      <div key={v.id} className="p-5 flex flex-col gap-3">
                         ...
                         <div className="flex items-center justify-between mt-1">
                            ...
                            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-slate-800 rounded-lg text-[10px] font-bold text-primary transition-all">
            ...
            <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[24px] sm:rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
               ...
               <div className="pt-4 border-t border-slate-50 dark:border-slate-800 grid grid-cols-2 gap-4">
                  <div>
                     ...
                     <p className="text-[11px] font-bold text-slate-600 dark:text-slate-400">{new Date(doc.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div>
                     <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mb-1">Update</p>
                     <p className="text-[11px] font-bold text-slate-600 dark:text-slate-400">{new Date(doc.updatedAt).toLocaleDateString()}</p>
                  </div>
               </div>
            </div>
